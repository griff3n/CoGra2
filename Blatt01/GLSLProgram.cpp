...
GLSLProgram::GLSLProgram(bool verbose) : handle(0), linked(false), logString(""), verbose(verbose) {}
...
bool GLSLProgram::compileShaderFromFile(const char* filename, GLSLShader::GLSLShaderType type) {
	...
	if (!file.good()) {
		logString = "could not open file \"" + string(filename) + "\"";
		return false;
	}
...
string GLSLProgram::log(void) const {
	return logString;
}
...
void GLSLProgram::setUniform(const char* name, int size, const mat4* value) {
	int location = getUniformLocation(name);
	if (location < 0) {
		if (verbose) {
			cout << "Uniform \"" << name << "\" not found" << endl;
		...
		glUniformMatrix4fv(location, size, GL_FALSE, value_ptr(value[0]));
	}
}
